...
DNode* CreateNode(DNode* pNode, int data); //노드를 생성하여 리턴한다.
DNode* FindNodeData(DNode* pStart, int data); //해당 데이터를 가진 노드를 찾는다.
DNode* InsertNodeData(DNode* pStart, int data, int insert); //해당 데이터를 가진 노드 뒤에 노드를 추가한다.
void DeleteNodeData(DNode* pStart, int del); //해당데이터를 가진 노드를 삭제한다.
void PrintLinkedList(DNode* pStart); //노드를 순회하며 끝날때까지 출력한다.
void DeleteLinkedList(DNode* pStart); //노드를 순회하며 모든데이터를 삭제한다.
...
void main()
{
	...
	DNode* pInsert = InsertNodeData(pBegin, 30, 60);//노드 삽입
	if (pInsert != NULL)
		printf("Insert:%d\n", pInsert->nData);
